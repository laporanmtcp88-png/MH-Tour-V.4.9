...
class MainActivity : ComponentActivity() {
    // AUTO ONLINE TEST: token dibuat oleh LiveKit Cloud Development Token Server.
    ...
    private fun home() {
        ...
        gb.setOnClickListener { if (requireInternet()) guideScreen() }; jb.setOnClickListener { if (requireInternet()) jamaahScreen() }
    }
    ...
    private fun guideScreen() {
        ...
        create.setOnClickListener {
            ...
            try {
                ...
                val generatedRoomName = "mh-tour-$generatedCode"
        ...
        talk.setOnClickListener {
            ...
            lifecycleScope.launch {
                try {
                    val session = parseSessionCode(currentCode ?: error("Sesi belum dibuat"))
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (result != null) {
            if (result.contents != null) {
                ...
                if (parts.size >= 4 && parts[0] == "MHTOUR" && parts[1] == "JOIN") {
                    ...
                    if (scannedCode.matches(Regex("UM[0-9]{6}")) && scannedRoom.startsWith("mh-tour-")) {
                        jamaahScreenWithCode(scannedCode)
                    } else {
    ...
    private data class SessionInfo(val code: String, val roomName: String)

    private fun parseSessionCode(value: String): SessionInfo {
        val code = value.trim().substringBefore("|").uppercase()
        require(code.matches(Regex("UM[0-9]{6}"))) { "Kode rombongan tidak valid" }
        // Room name is deterministic so a Guide can create it locally and a Jemaah
        // can join by typing only the six-digit code.
        return SessionInfo(code, "mh-tour-$code")
    }
